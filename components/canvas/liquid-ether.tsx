...
`;
    ...
    class ShaderPass {
      props: { material?: { vertexShader: string; fragmentShader: string; uniforms: Uniforms }; output?: THREE.WebGLRenderTarget };
      uniforms?: Uniforms;
    ...
    class ExternalForce extends ShaderPass {
      ...
      constructor(simProps: {
        ...
        super({ output: simProps.dst ?? undefined });
        this.init(simProps);
      }
      init(simProps: {
        cellScale: THREE.Vector2;
      ...
      update(props?: {
        ...
        const p = props || {};
        const forceX = (Mouse.diff.x / 2) * (p.mouse_force || 0);
        const forceY = (Mouse.diff.y / 2) * (p.mouse_force || 0);
        const cellScale = p.cellScale || new THREE.Vector2(1, 1);
        const cursorSize = p.cursor_size || 0;
        ...
        (uniforms.scale.value as THREE.Vector2).set(cursorSize, cursorSize);
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: {
        ...
        (this as unknown as { props: { output0?: THREE.WebGLRenderTarget; output1?: THREE.WebGLRenderTarget } }).props.output0 = simProps.dst_ ?? undefined;
        (this as unknown as { props: { output1?: THREE.WebGLRenderTarget } }).props.output1 = simProps.dst ?? undefined;
        this.init();
      }
      update(args?: { viscous?: number; iterations?: number; dt?: number }) {
        ...
        const self = this as unknown as {
          props: { output: THREE.WebGLRenderTarget; output0: THREE.WebGLRenderTarget; output1: THREE.WebGLRenderTarget };
          uniforms: Uniforms;
        };
        for (let i = 0; i < iter; i++) {
          const fbo_in = i % 2 === 0 ? self.props.output0 : self.props.output1;
          fbo_out = i % 2 === 0 ? self.props.output1 : self.props.output0;
    ...
    class Poisson extends ShaderPass {
      ...
      update(args?: { iterations?: number }) {
        ...
        for (let i = 0; i < iter; i++) {
          const p_in = i % 2 === 0 ? self.props.output0 : self.props.output1;
          p_out = i % 2 === 0 ? self.props.output1 : self.props.output0;
    ...
    class Simulation {
      ...
      update() {
        ...
        this.divergence.update({ vel: vel ?? undefined });
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: WebGLManager["props"]) {
        ...
        this.autoDriver = new AutoDriver(Mouse, this as unknown as WebGLManager, {
  ...
  return (
    <div
      ref={mountRef}
      className={`relative h-full w-full overflow-hidden pointer-events-none touch-none ${className}`}
      style={style}
    />
  );
}
